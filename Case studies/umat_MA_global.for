...
C
      DIMENSION XKIRCH(3,3), DUMSTRSS(3,3), XAMAT(3,2), XIF(2)
C           
...
C       Fibre vectors in the reference configuration
        XAMAT(1,1)=COS(thetar1);XAMAT(2,1)=SIN(thetar1);XAMAT(3,1)=ZERO
        XAMAT(1,2)=COS(thetar2);XAMAT(2,2)=SIN(thetar2);XAMAT(3,2)=ZERO
C
...
C     
     ...
     1 XAMAT, XI1, XI2, XIF, NPT, NANISO, iter)
C     
        STATEV(1) = XI1
        STATEV(2) = XI2
        STATEV(3) = XJ
